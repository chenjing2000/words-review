import json
from pathlib import Path


def resource_folder(wordlist_path: Path) -> Path:
    return wordlist_path.parent / wordlist_path.stem


def pronunciation_path(wordlist_path: Path) -> Path:
    return resource_folder(wordlist_path) / "pronunciation.json"


def load_pronunciations(wordlist_path: Path) -> dict[str, dict[str, list[str]]]:
    path = pronunciation_path(wordlist_path)
    if not path.exists():
        return {}

    try:
        with path.open("r", encoding="utf-8") as file:
            data = json.load(file)
    except (json.JSONDecodeError, UnicodeError) as exc:
        raise ValueError(f"发音配置文件格式错误: {path.name}") from exc

    if not isinstance(data, dict):
        raise ValueError("发音配置文件根节点必须是 JSON object")
    if data.get("schema_version") != 1:
        raise ValueError(f"不支持的 pronunciation schema_version: {data.get('schema_version')}")

    raw_words = data.get("words")
    if not isinstance(raw_words, dict):
        raise ValueError("pronunciation.json 缺少 words object")

    result = {}
    for word, item in raw_words.items():
        if not isinstance(word, str) or not isinstance(item, dict):
            continue

        accents = {}
        for accent in ("uk", "us"):
            value = item.get(accent, [])
            if isinstance(value, list):
                accents[accent] = [
                    path.strip()
                    for path in value
                    if isinstance(path, str) and path.strip()
                ]
            else:
                accents[accent] = []

        result[word] = accents

    return result


def find_audio_path(
    wordlist_path: Path,
    pronunciations: dict[str, dict[str, list[str]]],
    word: str,
    accent: str,
) -> Path | None:
    item = pronunciations.get(word)

    if item is None:
        folded = word.strip().casefold()
        for key, value in pronunciations.items():
            if key.strip().casefold() == folded:
                item = value
                break

    if not isinstance(item, dict):
        return None

    paths = item.get(accent, [])
    if not isinstance(paths, list):
        return None

    base = resource_folder(wordlist_path)
    for relative_path in paths:
        if not isinstance(relative_path, str) or not relative_path.strip():
            continue
        audio_path = base / relative_path
        if audio_path.is_file():
            return audio_path

    return None
